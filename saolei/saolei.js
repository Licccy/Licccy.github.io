 ...
 * 2.leftClick-->没有雷-->显示数字（代表以当前小格为中心周围8个格的雷数） 是0则扩散一片（当前周围8个格没有雷）
 *               有雷-->game over
 * 3.rightClick-->没有标记且无数字-->进行标记   有标记-->取消标记-->标记是否正确，10个都标记正确-->提示成功
 * 4.已经出现数字的-->无法标记
 */
...
function init () {
    ...
    // 生成100个小格,两个for循环，一个生成10行，一个生成10列
    for (var i = 0; i < 10; i++) {
        for (var j = 0; j < 10; j++) {
            var con = document.createElement('div');
            con.classList.add('block'); // 存放相同样式
            con.setAttribute('id', i + '-' + j); // 每个有自己的id：s记录当前行和列的值，第几行第几列
            box.appendChild(con);
            mineMap.push({mine:0}); // 最开始给每个小格雷的状态都标记为0
        }
    }
    // 生成雷-->100个里随机选10个
    block = document.getElementsByClassName('block');  // 类数组100个格
    while (minesNum) {
        // 随机标记一个雷要出现的位置
        var mineIndex = Math.floor(Math.random() * 100);
        // 判断随机的某个小格是否已经标记有雷，若已有，则不再在此位置标记雷，若没有则可以标记雷
